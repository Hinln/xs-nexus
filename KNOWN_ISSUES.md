...
- 已完成缓解：`/srv/xs-nexus/artifacts/qa/m5.1-20260730T232953Z` 覆盖错误签名、错误公钥、外层/内层篡改、防降级和回滚验证。Gate 18 候选 revision `b8cd49cf2be401cfe3b2d289a8cd1a50c3cc5bb1` 又以全绿 run `31515281011`、job `93858773221` 和 artifact `9110864186` 证明 schema 2 统一清单、一至四把唯一 PEM 公钥重叠、Controller 单向撤销、root-owned 本地撤销 ledger、下载/存储/平台失败、回滚执行前拒绝、身份与当前版本保持、真实 tmpfs ENOSPC 和 namespace 路由清理；归档/内部 SHA-256 和无值秘密扫描通过。Console 没有私钥上传字段。以上降低在线 Controller、暂存区替换和失败更新风险，但不替代正式签名仪式、认证生产分发、真实平台或生产发布链。
...
- 阻塞：轮换需要生产所有者控制的秘密渠道、当前凭据/云控制台、NAS/CI/真实节点管理权、正式维护窗口和独立验证方；生产 SSH host key 尚未带外确认。测试值、仓库代码或关闭严格 SSH 验证都不能代替这些条件。
- 计划：外部条件解除后，按 `audit/production-readiness-remediation-v2/CREDENTIAL_ROTATION.md` 逐项轮换，使用不回显渠道激活新值，从独立会话验证旧值拒绝，并扫描当前树、Git 历史、CI、镜像层、日志、截图和 QA 证据。
...
## KI-029 当前 revision 正式 24 小时长测尚未执行
