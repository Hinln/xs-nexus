...
- 计划：当前 IP Helper FFI、表释放、精确路由、非持久地址、DAD 有界等待、联合失败补偿、严格 manifest codec 与精确恢复计划已完成源码准备；下一步接入 `xs-windows-private-storage` 完成原子持久化、启动恢复执行和 rollback 失败诊断，再进入 Windows VM 实测。
