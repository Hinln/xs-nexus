...
## KI-009 M2.1 尚未证明真实公网和全部 NAT 行为

- 严重度：预期限制
- 状态：开放
- 首次发现：2026-07-29
- 影响：隔离 namespace 可以证明协议、候选优先级、映射端口一致性和认证路径迁移，但不能代表运营商网络、真实公网 IPv6、Full-cone、Restricted、Port-restricted、对称 NAT、双端 NAT 或 UDP 封锁行为。
- 复现：当前 M2.1 自动化使用临时 bridge、veth 和 namespace，不改变宿主机默认路由或 1Panel 防火墙。
- 临时缓解：只将 M2.1 描述为地址发现与候选管理完成，不宣称 NAT 打洞矩阵完成。
- 根因：真实 NAT 行为和打洞时序属于 M2.2；外部公网环境还需要受控测试节点和人工防火墙门禁。
- 计划：在 M2.2 建立可重复的 nftables NAT 模型，随后在获批外部测试节点补充真实公网验证。
- 解除条件：`EXECUTION_PLAN.md` 的 M2.2 网络实验矩阵具有实际证据，且任何外部测试均未伪造。
