...
## KI-012 Agent 路径遥测和 Relay 指标尚未接入控制台
...
- 已完成缓解：节点在线/离线计数只来源于当前进程已认证控制连接；Relay 本地指标端点已经真实提供字节、可观测丢弃、I/O 错误和队列转发延迟，且不记录 payload 或节点身份。
- 证据：控制台状态 `/srv/xs-nexus/artifacts/qa/m4.2-20260730T223401Z`；Relay 指标 `/srv/xs-nexus/artifacts/qa/m2.3-20260731T185454Z`。
- 计划：在性能与可观测性阶段定义有界、认证、脱敏的 Agent 指标上报与保留策略，并为 Controller 配置受控 Relay 指标采集端点，再接入管理快照。
