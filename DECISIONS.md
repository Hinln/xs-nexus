...
- 决策：每个目标产物由归档、固定 schema 2 十二字段外部清单和 Ed25519 分离签名组成。清单额外绑定精确源码提交、正整数 source-date epoch 和 `XSP/1` 协议版本。安装器必须先验证签名再解析清单，随后验证产品、版本、来源、平台、架构、目标、文件名、长度和 SHA-256。解包前后执行固定成员 allowlist、类型检查和精确逐文件哈希；不接受链接、设备、额外成员或未签名安装脚本。
...
## ADR-096：发布撤销必须同时约束在线调度与离线主机执行边界

- 状态：接受
- 日期：2026-08-12
- 背景：仅从 Controller 停止下发不能阻止已暂存请求、控制链路中断或根辅助程序竞态；仅依赖主机文件又不能让管理策略、审计和节点调度及时收敛。现有自动更新还只能解析 schema 1，而当前发布构建器已经输出绑定来源信息的 schema 2，导致真实发布产物无法进入自动更新链路。
...
## ADR-101：当前 revision 长测以正式时长、进程代际和可验证主机身份为硬边界
...
- 证据：最新 exact revision `795b1ea461a179958aed27e6935faba8f36e43ce` 的 run `31658778589` 全 13 job 通过；job `94318968217`、artifact `9165687987`、84/84 内部 SHA-256、零秘密扫描、八项事件、258 个资源样本和九类前后不变量通过。
...
- 证据：exact revision `795b1ea461a179958aed27e6935faba8f36e43ce`，run `31658778589`，job `94318968293`，artifact `9165661918`，110+8 清单、零秘密、10/10 阶段和 20/20 不变量通过；性能 artifact `9165514010` 的 Direct/Relay 正式 p95 为 `0.408`/`0.465` ms。
