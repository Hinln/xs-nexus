...
- 代价：当前已实现隔离 `GetIpForwardTable2`/`FreeMibTable`、精确 Create/Delete 路由、非持久地址 Create/Delete、DAD 查询和有界轮询，以及地址成功后才执行的路由联合事务；DAD 拒绝/超时/查询失败与路由失败都会精确删除地址并显式返回清理失败。已通过 MSVC target check/Clippy；尚未实现 manifest 持久化、崩溃恢复或 Windows 执行，不能描述为真实路由管理。
