...
- Agent Windows 网络准备层固定 stale recovery→Preparing→DAD→routes→Active 顺序，shutdown 只在完整清理后删除 manifest；runtime 源码门禁禁止引用，避免无 VM 证据时启用主机路由。LUID 作为显式参数传入，不按可注入名称查找。源码门禁与存储回归通过，但该 Agent 模块尚未越过完整 Windows SDK 链接门禁。
