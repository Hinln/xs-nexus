...
- 控制台接入真实网络、节点、Token、组、ACL、子网、Relay、拓扑、用户、审计和告警数据；当时未接入的遥测明确显示“未采集”。后续已接入 Agent/Relay 身份签名的路径、流量、延迟和健康指标，缺失或陈旧报告仍不推断默认值；
