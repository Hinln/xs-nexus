...
当前已完成跨平台 ABI、数据面和生命周期模型、UMDF/NetAdapterCx 源码、Rust Agent 客户端契约、严格测试安装器、显式 MSBuild/InfVerif/Inf2Cat/SignTool 测试包构建脚本、六阶段快照 VM 采证编排，以及 exact ABI v1/INF DriverVer/driver-store/安装状态 schema 2 一致性门禁。测试安装明确只支持 clean install/uninstall，不声称热升级或生产回滚。最新自动化证据为 `/srv/xs-nexus/artifacts/qa/m6.1-compatibility-20260731T022619Z`；没有 Windows Rust/SDK/WDK/VM，M6.1 保持 `IN_PROGRESS`，M6.2 保持 `BLOCKED_EXTERNAL`。
