...
QA_DIR="$ROOT_DIR/artifacts/qa/m4.2-$STAMP"
LOG_FILE="$QA_DIR/validate-m42.log"
...
printf 'M4.2 validation started at %s\n' "$(date -u --iso-8601=seconds)"
printf 'git_head=%s\n' "$(git rev-parse HEAD)"
...
make test-e2e
make test-visual
visual_count=$(find artifacts/visual/m4.1 -type f -name '*.png' | wc -l)
printf 'visual_screenshot_count=%s\n' "$visual_count"
test "$visual_count" -ge 135
make security-check
...
for interface in xsa0 xsb0 xstest0; do
...
printf 'M4.2 validation passed at %s\n' "$(date -u --iso-8601=seconds)"
