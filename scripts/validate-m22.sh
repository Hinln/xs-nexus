...
QA_DIR="$ROOT_DIR/artifacts/qa/m2.2-$STAMP"
LOG_FILE="$QA_DIR/validate-m22.log"
...
printf 'M2.2 validation started at %s\n' "$(date -u --iso-8601=seconds)"
...
if ip netns list | awk '{print $1}' |
    grep -Eq '^(xsm13|xsm2f|xsm21|xsm22|x22)'; then
    ip netns list >&2
    exit 1
fi
if ip -o link show | awk -F': ' '{print $2}' | cut -d@ -f1 |
    grep -Eq '^(xm13|xmm2f|xm21|xm22|m[1-6](a|b|r|w|n))'; then
    ip -o link show >&2
    exit 1
fi
...
printf 'M2.2 validation passed at %s\n' "$(date -u --iso-8601=seconds)"
