...
QA_DIR="$ROOT_DIR/artifacts/qa/m3.1-$STAMP"
LOG_FILE="$QA_DIR/validate-m31.log"
...
printf 'M3.1 validation started at %s\n' "$(date -u --iso-8601=seconds)"
printf 'git_head=%s\n' "$(git rev-parse HEAD)"
...
if ip netns list | awk '{print $1}' |
    grep -Eq '^(xsm13|xsm2f|xsm21|xsm22|xsm23|xsm31|x22)'; then
    ip netns list >&2
    exit 1
fi
...
printf 'M3.1 validation passed at %s\n' "$(date -u --iso-8601=seconds)"
