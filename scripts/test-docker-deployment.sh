...
BAD_DATABASE_SECRETS="$TEMPORARY/bad-database"
...
write_environment() {
    local path=$1 controller_secrets=$2 database_secrets=$3 controller_image=$4 database_schema=$5
    cat >"$path" <<EOF
XS_DEPLOYMENT=dev
...
chmod 0700 "$CONTROLLER_SECRETS" "$BAD_CONTROLLER_SECRETS" "$DATABASE_SECRETS" "$BAD_DATABASE_SECRETS" "$RELAY_SECRETS" "$BACKUP_DIRECTORY" "$REPLICA_DIRECTORY" "$STATE_DIRECTORY"
...
chown -R 65532:65532 "$DATABASE_SECRETS"
chown 65532:65532 "$REPLICA_DIRECTORY/.xs-nexus-replica"
find "$CONTROLLER_SECRETS" "$DATABASE_SECRETS" "$RELAY_SECRETS" -type f -exec chmod 0400 {} +
...
cp -a "$DATABASE_SECRETS/." "$BAD_DATABASE_SECRETS/"
printf '%s' 'postgresql://127.0.0.1:1/invalid' >"$BAD_DATABASE_SECRETS/database-url"
chown -R 65532:65532 "$BAD_CONTROLLER_SECRETS"
chown -R 65532:65532 "$BAD_DATABASE_SECRETS"
find "$BAD_CONTROLLER_SECRETS" "$BAD_DATABASE_SECRETS" -type f -exec chmod 0400 {} +
