...
- [x] Gate 25 repository clean-release submatrix passes at exact code candidate `a44d868` with an exact detached checkout, test-only tag, reproducible images, real Direct/Relay/subnet/update/deployment lifecycles, exact cleanup, and independently verified evidence; the gate remains `FAIL` pending a formal owner-signed RC and independent production rehearsal.
...
Exact code candidate `a44d868c26e21285565fa794d482b8092c0bbbf4` passed all 14 jobs in run `31671264821`. The 13 downloaded artifacts passed 18 manifests, 1,027 independent hashes and a whole-artifact zero-finding secret scan. The retained prior run `31670477509` closed two harness-integrity defects without assertion weakening. Native Windows compilation is checked as repository prerequisite evidence, but its artifact records no device installation or Driver Verifier. The current value remains `NO` because every unchecked item above is still release-governing.

While waiting for owner-controlled environments, use `EXTERNAL_GATE_HANDOFF.md` and `scripts/external-gate-kit.py` to create the no-secret evidence kit outside Git. A fully sealed kit reports only `READY_FOR_FRESH_AUDIT`; it cannot check these boxes by itself or change the current `NO` value. Each checkbox still requires the actual external operation and an independent fresh 25-gate audit.
