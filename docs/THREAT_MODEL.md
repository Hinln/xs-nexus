...
| T13 | 数据库/Redis 暴露 | 凭据攻击、数据泄露 | 仅容器网络、最小用户、无公网端口、TLS/认证、备份 | 当前生产候选已解除 KI-006；仍需持续监控端口映射漂移和凭据轮换 |
...
| T17 | Windows 驱动边界错误 | 提权、蓝屏 | 最小 IOCTL、长度/整数/生命周期验证、Driver Verifier、VM 快照 | 测试签名驱动实验室门禁已通过；完整 Agent、生产签名/安装升级、Windows 10 与更广生命周期矩阵完成前不可发布 |
...
| 遥测 | Agent/Relay 正常签名、错误签名、重放、陈旧/未来时间、计数回滚、集合/分类不一致、样本保留和 Console 缺失/陈旧状态 |
| 主机恢复 | Agent 崩溃、卸载、默认路由不变、项目规则无残留 |
| 更新 | 清单篡改、哈希错误、版本回滚、失败恢复 |
| 备份 | 密文篡改、错误 identity、公开/深度校验、异地取回、恢复回滚、保留/销毁和同名复用拒绝 |
...
- 全球级流量分析和强制断网；
- 尚未完成的第三方协议、密码学和驱动审计；
- 未经用户门禁完成的真实 NAS、完整 Windows Agent/runtime/正式签名、DNS 和生产防火墙。Windows 11 测试签名 xsnet 驱动 VM/Verifier 门禁已单独完成，不外推到这些剩余范围。
