...
| T20 | 资源耗尽 | 服务不可用 | 报文长度上限、会话配额、廉价预检、验签前全局注册预算、每 Lease 与全局包/字节队列及速率限制 | 分布式有效节点和大规模 DDoS 仍需云侧缓解 |
...
Relay 在扩展任何来源状态或执行注册公钥验签前消耗全局预算；数据队列同时受每目标 Lease 和全局 packet/byte 上限约束。拒绝发生在重放窗口与流量预算提交前，过期、替换和清理必须释放精确全局计数。这些控制限制单进程内可归因资源消耗，但不声称抵御云侧链路饱和、分布式合法凭据、跨地域放大或宿主资源耗尽。
