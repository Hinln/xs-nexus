...
export const snapshotFixture: ConsoleSnapshot = {
  ...
  dashboard: {
    online_nodes: 1,
    offline_nodes: 1,
    direct_nodes: unavailable<number>("Agent 尚未上报当前路径遥测"),
    relay_nodes: unavailable<number>("Agent 尚未上报当前路径遥测"),
    relay_health: unavailable<string>("Relay 目录未包含健康指标端点"),
    traffic_bytes_24h: unavailable<number>("Agent 尚未上报流量遥测"),
    connection_success_percent_24h: unavailable<number>("Agent 尚未上报连接结果遥测"),
    average_latency_ms_24h: unavailable<number>("Agent 尚未上报路径延迟遥测"),
  ...
  relays: [
    {
      ...
      health: unavailable<string>("Relay 健康指标端点未配置"),
      metrics: unavailable<Record<string, number>>("Relay 指标端点未配置"),
    },
  ],
  topology: {
    nodes: [
    ...
    link_telemetry: unavailable<string>("Agent 尚未上报已选链路"),
  },
  audit_events: [
    {
      id: 22,
