...
- `scripts/windows/build-xsnet-test-package.ps1` 只在显式确认的 Windows 11 26100+ 管理员测试目标中运行；VM 模式要求可识别 VM，实体机模式要求专用目标与恢复就绪声明。固定 Microsoft-signed MSBuild/InfVerif/Inf2Cat/SignTool、Release x64、`SignMode=Off`、先嵌入签名 DLL 再生成并签名 `10_GE_X64` catalog、SHA-256 test signer 和精确 INF/CAT/DLL allowlist；不修改 BCD、信任根或测试签名策略；
- `scripts/windows/invoke-xsnet-test-vm-stage.ps1` 以 Initialize/Install/EnableVerifier/CollectVerifier/DisableVerifier/Uninstall 六个不可复用阶段保存系统、网卡、路由、设备、驱动、Verifier 和错误事件证据；VM 要求同一快照，实体机要求同一外置系统镜像、恢复介质、磁盘恢复材料与现场恢复人员标识；两种模式均要求 Verifier 前后两次人工重启、精确健康状态和卸载零残留，最终生成 SHA-256 证据清单；
...
- 测试安装器不支持 in-place upgrade；替换包只能从获批恢复基线停止 Agent、关闭 handle、精确卸载后 clean install。VM 使用快照；实体目标使用外置完整系统镜像、可启动恢复介质、磁盘恢复材料和现场恢复人员。兼容/回滚边界证据为 `/srv/xs-nexus/artifacts/qa/m6.1-compatibility-20260731T022619Z`，真实版本升级、回滚和跨 ABI 拒绝仍未执行；
